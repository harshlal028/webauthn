...
* Because the website had an ongoing conditional mediation assertion with this extension set, the user agent choses to automatically create a passkey for the account. This credential given to the RP via fulfilling the promise returned by `credentials.navigator.get` with an `AuthenticatorAttestationResponse.`
...
To inform the user agent that it's desired for a credential to be created whenever an authentication ceremony is mediated via non-WebAuthn means.
...
if (assertionOrRegistration.attestationObject) {
  // A credential was created.
} else {
  // A credential was asserted.
}
```

In the case where the registration is completed, the promise returned by navigator.credentials.get will complete with an AuthenticatorAttestationResponse instead of an AuthenticatorAssertionResponse.
